...
def _aggregate_none_metric(
    ...
    if time_range == "D":
        ...
        freq = "W"
        full_range = pd.date_range(window_start, now, freq=freq)
        # fmt = lambda ts: f"Week {((ts.day - 1) // 7) + 1}"
...
def detail_page(title, metric_id: int):
    ...
    with ui.card().classes("q-mb-md w-full"):
        with ui.card_section().classes("w-full"):
            chart_title = ui.label("Count by Day").classes("text-h6 color-6 q-mb-sm")
            chart = (
                ui.echart(
                    _build_chart_options(metric, *_aggregate_none_metric(logs, "W", tz))
                )
                .classes("w-full")
                .style("height: 250px")
            )
